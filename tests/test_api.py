...
def test_predict():
    payload = {'input_text': 'Привет, я твой друг, я тебя не обижу'}
...
def test_batch_predict():
    payload = {'input_texts': ['Привет, я твой друг, я тебя не обижу', 'Привет! Я твой друг! Я тебя не обижу!']}
...
def test_empty_predict_batch():
    payload = {'input_texts': None}
    response = client.post('/predict', json=payload)
    assert response.status_code == 422


def test_empty_predict():
    payload = {'input_text': None}
    response = client.post('/predict', json=payload)
    assert response.status_code == 422