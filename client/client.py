...
import numpy as np
from client.schemas import ClassificationRequestBatch, ClassificationResponseBatch
...
async def predict(input_texts: ClassificationRequestBatch) -> ClassificationResponseBatch:
    ...
    texts_array = np.array([text.encode('utf-8') for text in input_texts], dtype=object).reshape(-1, 1)
